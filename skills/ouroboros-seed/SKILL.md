---
name: ouroboros-seed
description: Ouroboros seed workflow. Generate validated Seed specifications from interview results. Use when running the seed phase of a specification-first Ouroboros loop in Codex.
...
1. Use the `ToolSearch if available` tool to find and load the seed generation MCP tool:
   ```
   ToolSearch if available query: "+ouroboros seed"
   ```
2. The tool will typically be named `mcp__plugin_ouroboros_ouroboros__ouroboros_generate_seed` (with a plugin prefix). After ToolSearch if available returns, the tool becomes callable.
3. If ToolSearch if available finds the tool → proceed to **Path A**. If not → proceed to **Path B**.

**IMPORTANT**: Do NOT skip this step. Do NOT assume MCP tools are unavailable just because they don't appear in your immediate tool list. They are almost always available as deferred tools that need to be loaded first.

### Path A: MCP Mode (Preferred)

If the `ouroboros_generate_seed` MCP tool is available (loaded via ToolSearch if available above):
...
Then check `~/.ouroboros/prefs.json` for `star_asked`. If `star_asked` is not set to `true`, use the **ask the user tool** with this single question:
