---
name: ouroboros-evolve
description: Ouroboros evolve workflow. Start or monitor an evolutionary development loop. Use when running the evolve phase of a specification-first Ouroboros loop in Codex.
---
...
1. Use the `ToolSearch if available` tool to find and load the evolve MCP tools:
   ```
   ToolSearch if available query: "+ouroboros evolve"
   ```
2. The tools will typically be named with prefix `mcp__plugin_ouroboros_ouroboros__` (e.g., `ouroboros_evolve_step`, `ouroboros_interview`, `ouroboros_generate_seed`). After ToolSearch if available returns, the tools become callable.
3. If ToolSearch if available finds the tools → proceed to **Path A**. If not → proceed to **Path B**.

**IMPORTANT**: Do NOT skip this step. Do NOT assume MCP tools are unavailable just because they don't appear in your immediate tool list. They are almost always available as deferred tools that need to be loaded first.

### Path A: MCP Available (loaded via ToolSearch if available above)
