---
name: ouroboros-unstuck
description: Ouroboros unstuck workflow. Break through stagnation with lateral thinking personas. Use when running the unstuck phase of a specification-first Ouroboros loop in Codex.
---
...
1. Use the `ToolSearch if available` tool to find and load the lateral thinking MCP tool:
   ```
   ToolSearch if available query: "+ouroboros lateral"
   ```
2. The tool will typically be named `mcp__plugin_ouroboros_ouroboros__ouroboros_lateral_think` (with a plugin prefix). After ToolSearch if available returns, the tool becomes callable.
3. If ToolSearch if available finds the tool → use MCP mode below. If not → skip to **Fallback** section.
