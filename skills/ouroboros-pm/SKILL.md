---
name: ouroboros-pm
description: Ouroboros pm workflow. Generate a PM through guided PM-focused interview with automatic question classification. Use when the user says 'ooo pm', 'prd', 'product requirements', or wants to create a PRD/PM document.. Use when running the pm phase of a specification-first Ouroboros loop in Codex.
---
...
```
ToolSearch if available query: "+ouroboros pm_interview"
```
...
- **YES** → Pass it directly to `ask the user`:
  ```
  ask the user(questions=[meta.ask_user_question])
  ```
  Do NOT modify it. Do NOT add options. Do NOT rephrase the question.

- **NO** → This is an interview question. Use `ask the user` with `meta.question`.
