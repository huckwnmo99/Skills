...
description: Ouroboros status workflow. Check session status and measure goal drift. Use when running the status phase of a specification-first Ouroboros loop in Codex.
...
1. Use the `ToolSearch if available` tool to find and load the status MCP tools:
   ```
   ToolSearch if available query: "+ouroboros session status"
   ```
2. The tools will typically be named with prefix `mcp__plugin_ouroboros_ouroboros__` (e.g., `ouroboros_session_status`, `ouroboros_measure_drift`). After ToolSearch if available returns, the tools become callable.
